...
type CacheTable struct {
	...
	// add by lucas, Callback method triggered before expire an item from the cache
	onExpired func(item *CacheItem)
}
...
// Add by Lucas
func (table *CacheTable) SetOnExpiredCallback(f func(*CacheItem)) {
	table.Lock()
	defer table.Unlock()
	table.onExpired = f
}
...
func (table *CacheTable) expirationCheck() {
	...
	for key, item := range table.items {
		...
		if now.Sub(accessedOn) >= lifeSpan {
			...
			table.deleteInternal(key, Expired)
		} else {
...
func (table *CacheTable) deleteInternal(key interface{}, reason RemoveReason) (*CacheItem, error) {
	...
	if reason == Expired {
		if r.aboutToExpire != nil {
			for _, callback := range r.aboutToExpire {
				callback(r) // modify by Lucas
			}
		} else { // add by Lucas
			if table.onExpired != nil {
				table.onExpired(r)
			}
		}
	}
...
func (table *CacheTable) Delete(key interface{}) (*CacheItem, error) {
	...
	return table.deleteInternal(key, Deleted)
}
